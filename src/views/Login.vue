				...
				<div class="card">
					<div class="card-body">
						...
						<div class="app-brand justify-content-center">
							<router-link :to="{path:'/'}" class="app-brand-link gap-2">
						...
						<!-- /Logo -->
						<h4 class="mb-2">欢迎使用xxx系统! 👋</h4>
						<p class="mb-4">请先登录系统后，再开启冒险之旅</p>
						<form @submit.prevent="login" id="formAuthentication" class="mb-3" method="POST">
							<div class="mb-3">
								<label for="email" class="form-label">用户名</label>
								<input v-model="formData.userName" type="text" :class="['form-control', {'border-danger':errors.userName}]" id="userName" name="userName" placeholder="请输入用户名" @blur="validateField('userName', formData.userName)" />
							</div>
							<div class="mb-3 form-password-toggle">
								...
								<div class="input-group input-group-merge">
									<input v-model="formData.password" type="password" id="password" :class="['form-control', {'border-danger':errors.password}]" name="password" placeholder="请输入密码" aria-describedby="password" @blur="validateField('password', formData.password)"/>
							...
							<div class="mb-3">
								<label for="btn" v-if="isLoginError" class="form-label text-danger">{{ loginErrorMsg}}</label>
								<button class="btn btn-primary d-grid w-100" type="submit" :disabled="isLogging">
